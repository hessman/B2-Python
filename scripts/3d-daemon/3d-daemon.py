...
parser = argparse.ArgumentParser(description="A simple daemon backup tool. " +
                                             "Configuration in " + config_json,
                                 epilog="./3d-daemon.py -s ./things ./other_things -o ./data -p 22 -a 192.168.56.100 "
                                        + "-u root -P 123 -D /var/backup -r Or for daemon use ./3d-daemon.py -d",
                                 prog="3d-daemon")
...
# TODO : If daemon -> provision variables with config.json
quiet = args.quiet
...
def get_directory_size(directory: str):
    """
    Give the size of the given directory.
    :param directory: Path of the directory to get size.
...
if not daemon_mode:
    ...
    write_stdout("comming soon !")
    # TODO : Infinite loop with sleep inside
        # TODO : For all dirs
            # TODO : check size and compare with config.json
                # TODO : local backup if oversized + rm files
                # TODO : ssh backup if needed
